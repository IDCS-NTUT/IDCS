...
class GstAppSrcWriter:
    def __init__(self, pipeline: str, fps: int, width: int, height: int) -> None:
        ...
        self._pts = 0
        self._opened = False
        self._last_flow_return: Optional[Gst.FlowReturn] = None
    ...
    def write(self, frame: np.ndarray) -> bool:
        ...
        if result != Gst.FlowReturn.OK:
            self._last_flow_return = result
            print(f"[gst] appsrc push-buffer returned {result}")
            return False
        return True

    @property
    def last_flow_return(self) -> Optional[Gst.FlowReturn]:
        return self._last_flow_return
