...
- **F6 coalescing (latest-wins)**: update commands with function `F6` are coalesced by
  `(target, addr, func)`. If another F6 for the same key is already queued, it is replaced
  with the newest payload before execution.
- **Critical commands are never coalesced**: `F7` and stop/disable commands are always kept
  as independent queue entries.
...
- Applies only to `F6` commands.
- A command is stale when `now_ms - sent_ts_ms > f6_stale_threshold_ms`.
- `sent_ts_ms` can be provided by producers; otherwise ingest time is used.
