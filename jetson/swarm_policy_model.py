...
class SwarmPolicyNetwork(nn.Module):
    """Scores each target in a masked set and outputs one logit per target."""
    ...
    def forward(
        ...
        """Return masked target logits with shape ``[batch, num_targets]``."""
        ...
        return logits.masked_fill(~target_mask_bool, invalid_fill)
...
class SwarmPolicySelector:
    ...
    def predict_action(
        ...
        *,
        ...
        with torch.no_grad():
            logits = self.forward(target_features, global_features, target_mask)
            if return_probs:
    ...
    def predict_action_numpy(
        ...
        with torch.no_grad():
            logits = self.forward(target_tensor, global_tensor, mask_tensor)
