...
class SwarmControllerIntegrationTests(unittest.TestCase):
    ...
    def test_swarm_selector_falls_back_to_tracked_candidate_before_engageable(self) -> None:
        cfg = ControlConfig(
            mode="rate",
            loop_hz=30.0,
            fx_px=800.0,
            ...
            swarm_eval=SwarmEvalConfig(enabled=True, max_engage_distance_m=25.0),
        )
        loop = ControlLoop(cfg, _DummyPub())
        msg = DetectionMsg(
            frame_id=1,
    ...
    def test_track_mode_reports_loss_when_existing_target_disappears(self) -> None:
        cfg = ControlConfig(
            mode="rate",
            loop_hz=30.0,
            fx_px=800.0,
        ...
        loop.update_detection(reacquired)

        self.assertEqual(reacquired.target_track_id, 10)
        self.assertEqual(reacquired.target_idx, 0)

    def test_track_mode_does_not_index_lock_untracked_distractor(self) -> None:
        cfg = ControlConfig(
            mode="rate",
            loop_hz=30.0,
            fx_px=800.0,
    ...
    def test_track_mode_prefers_single_id_bearing_crop_box(self) -> None:
        cfg = ControlConfig(
            mode="rate",
            loop_hz=30.0,
            fx_px=800.0,
