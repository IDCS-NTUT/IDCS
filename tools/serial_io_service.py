...
def main() -> int:
    ...
    update_endpoint = endpoints_cfg.get("update_endpoint") or serial_cfg.get("update_endpoint")
    if not update_endpoint:
        update_endpoint = net_cfg.get("zmq_serial_update") or args.update_endpoint

    reply_endpoint = endpoints_cfg.get("reply_endpoint") or serial_cfg.get("reply_endpoint")
    ...
    rep.bind(command_endpoint)

    sub = ctx.socket(zmq.SUB)
    sub.setsockopt(zmq.LINGER, 0)
    sub.setsockopt_string(zmq.SUBSCRIBE, "")
    sub.bind(update_endpoint)

    pub = ctx.socket(zmq.PUB)
    pub.setsockopt(zmq.LINGER, 0)
    pub.bind(reply_endpoint)
