...
def _decode_update(data: bytes) -> List[SerialCommand]:
    ...
    payload_sent_ts_raw = payload.get("sent_ts_ms")
    payload_sent_ts_ms = int(payload_sent_ts_raw) if payload_sent_ts_raw is not None else ingest_ts_ms
    ...
    for entry in payload.get("commands", []):
        ...
        entry_sent_ts_raw = entry.get("sent_ts_ms")
        entry_sent_ts_ms = (
            int(entry_sent_ts_raw) if entry_sent_ts_raw is not None else payload_sent_ts_ms
        )
        try:
            cmd = SerialCommand(
                cmd_id=str(cmd_id),
